...
public class TieredSpilloverCache<K, V> implements ICache<K, V> {
    ...
    TieredSpilloverCache(Builder<K, V> builder) {
        ...
        this.heapStats = new StatsHolder(
            builder.cacheConfig.getDimensionNames(),
            builder.cacheConfig.getSettings(),
            StatsHolder.TrackingMode.ALL_COMBINATIONS
        );
        this.diskStats = new StatsHolder(
            builder.cacheConfig.getDimensionNames(),
            builder.cacheConfig.getSettings(),
            StatsHolder.TrackingMode.ALL_COMBINATIONS
        );
    ...
    public CacheStats stats() {
        return new TieredSpilloverCacheStats(heapStats, diskStats);
    }
        ...
        return key -> {
            try (ReleasableLock ignore = readLock.acquire()) {
                for (Tuple<ICache<K, V>, StatsHolder> pair : cacheAndStatsList) {
                    ...
                    if (value != null) {
                        pair.v2().incrementHitsByDimensions(key.dimensions);
                        return value;
                    } else {
                        pair.v2().incrementMissesByDimensions(key.dimensions);
                    }
                }
            }
            return null;
    ...
    void updateStatsOnRemoval(StatsHolder statsHolder, boolean wasEvicted, ICacheKey<K> key, V value) {
        if (wasEvicted) {
            statsHolder.incrementEvictionsByDimensions(key.dimensions);
        }
