...
import static org.opensearch.cache.common.tier.TieredSpilloverCacheStats.DISK_DIMS;
import static org.opensearch.cache.common.tier.TieredSpilloverCacheStats.HEAP_DIMS;
...
public class TieredSpilloverCache<K, V> implements ICache<K, V> {
    ...
    TieredSpilloverCache(Builder<K, V> builder) {
        ...
        this.onHeapCache = builder.onHeapCacheFactory.create(
            new CacheConfig.Builder<K, V>()
                .setRemovalListener(onHeapRemovalListener)
        ...
        this.diskCache = builder.diskCacheFactory.create(
            new CacheConfig.Builder<K, V>()
                .setRemovalListener(onDiskRemovalListener)
                .setKeyType(builder.cacheConfig.getKeyType())
                .setValueType(builder.cacheConfig.getValueType())
                .setSettings(builder.cacheConfig.getSettings())
                .setWeigher(builder.cacheConfig.getWeigher())
        ...
        this.stats = new TieredSpilloverCacheStats(dimensionNames);
    }
    ...
    public void put(ICacheKey<K> key, V value) {
        try (ReleasableLock ignore = writeLock.acquire()) {
            ...
            updateStatsOnPut(getKeyDimensionsWithTier(key, onHeapCache), key, value);
        }
    }
        ...
        try (ReleasableLock ignore = writeLock.acquire()) {
            for (ICache<K, V> cache : cacheList) {
                ...
                cache.invalidate(key);
                    //stats.decrementEntriesByDimensions(getKeyDimensionsWithTier(key, cache));
                //}
            }
        }
    }

    private List<CacheStatsDimension> getKeyDimensionsWithTier(ICacheKey<K> key, ICache<K, V> cache) {
        List<CacheStatsDimension> dimensions = new ArrayList<>(key.dimensions);
        dimensions.add(getTierDimensionForCache(cache));
        return dimensions;
    }

    private CacheStatsDimension getTierDimensionForCache(ICache<K, V> cache) {
    ...
    private Function<ICacheKey<K>, V> getValueFromTieredCache() {
        return key -> {
            try (ReleasableLock ignore = readLock.acquire()) {
                for (ICache<K, V> cache : cacheList) {
                    List<CacheStatsDimension> dimensions = new ArrayList<>(key.dimensions);
                    CacheStatsDimension tierDimension = getTierDimensionForCache(cache);
                    dimensions.add(tierDimension);
    ...
    void handleRemovalFromHeapTier(RemovalNotification<ICacheKey<K>, V> notification) {
        ...
        List<CacheStatsDimension> dimensions = getKeyDimensionsWithTier(key, onHeapCache);
        ...
        if (RemovalReason.EVICTED.equals(notification.getRemovalReason())
            ...
            try (ReleasableLock ignore = writeLock.acquire()) {
                ...
                updateStatsOnPut(getKeyDimensionsWithTier(key, diskCache), key, notification.getValue());
            }
            wasEvicted = true;
        }
        ...
        updateStatsOnRemoval(dimensions, wasEvicted, key, notification.getValue());
    }
    ...
    void handleRemovalFromDiskTier(RemovalNotification<ICacheKey<K>, V> notification) {
        ...
        List<CacheStatsDimension> dimensions = getKeyDimensionsWithTier(notification.getKey(), diskCache);
        boolean wasEvicted = false;
        if (RemovalReason.EVICTED.equals(notification.getRemovalReason())
            || RemovalReason.CAPACITY.equals(notification.getRemovalReason())) {
            wasEvicted = true;
        }
        updateStatsOnRemoval(dimensions, wasEvicted, notification.getKey(), notification.getValue());
    }

    void updateStatsOnRemoval(List<CacheStatsDimension> dimensions, boolean wasEvicted, ICacheKey<K> key, V value) {
        if (wasEvicted) {
            stats.incrementEvictionsByDimensions(dimensions);
        }
        stats.decrementEntriesByDimensions(dimensions);
        stats.incrementMemorySizeByDimensions(dimensions, -weigher.applyAsLong(key, value));
    }

    void updateStatsOnPut(List<CacheStatsDimension> dimensions, ICacheKey<K> key, V value) {
