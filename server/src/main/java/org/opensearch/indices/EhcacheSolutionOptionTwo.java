 ...
 * This implementation passes serializers of type (T -> byte[]) for use outside of ehcache,
 * which will use its own bundled byte[] serializer implementation.
 */
...
public class EhcacheSolutionOptionTwo<K, V> implements DiskCachingTier<K, V> {

    // A Cache manager can create many caches.
    private final PersistentCacheManager cacheManager;

    // Disk cache
    private org.ehcache.Cache<K, byte[]> cache;
    // for this test, only the value is byte[], since we have to compare fairly to other implementations with the key being the default ehcache Integer serializer
    ...
    public final Serializer<V, byte[]> valueSerializer;
    public CountDownLatch latch; // for benchmarking only
    private EhcacheSolutionOptionTwo(Builder<K, V> builder) {
    ...
    private org.ehcache.Cache<K, byte[]> buildCache(Duration expireAfterAccess, Builder<K, V> builder) {
        CacheConfigurationBuilder<K, byte[]> cacheBuilder = CacheConfigurationBuilder.newCacheConfigurationBuilder(this.keyType,
                byte[].class,
                ResourcePoolsBuilder.newResourcePoolsBuilder().disk(maxWeightInBytes, MemoryUnit.B)).withExpiry(new ExpiryPolicy<K, byte[]>() {
                @Override
...
//        }
    ...
    class EhCacheEventListener<K, V> implements CacheEventListener<K, V> {
        ...
        private final EhcacheSolutionOptionTwo<K, V> tier; // used for countdown latch in benchmarking only
        EhCacheEventListener(EhcacheSolutionOptionTwo<K, V> tier) {
            this.tier = tier;
        }
    ...
    class EhCacheKeyIterator<K> implements Iterator<K> {
        ...
        Iterator<org.ehcache.Cache.Entry<K, byte[]>> iterator;
    ...
    public static class Builder<K, V> {
        ...
        public EhcacheSolutionOptionTwo.Builder<K, V> setMaximumWeightInBytes(long sizeInBytes) {
            //assert sizeInBytes > 1024*100;
            this.maxWeightInBytes = sizeInBytes;
            return this;
        }

        public EhcacheSolutionOptionTwo.Builder<K, V> setExpireAfterAccess(TimeValue expireAfterAcess) {
            this.expireAfterAcess = expireAfterAcess;
            return this;
        }

        public EhcacheSolutionOptionTwo.Builder<K, V> setKeyType(Class<K> keyType) {
            this.keyType = keyType;
            return this;
        }

        public EhcacheSolutionOptionTwo.Builder<K, V> setValueType(Class<V> valueType) {
            this.valueType = valueType;
            return this;
        }

        public EhcacheSolutionOptionTwo.Builder<K, V> setStoragePath(String storagePath) {
            this.storagePath = storagePath;
            return this;
        }

        public EhcacheSolutionOptionTwo.Builder<K, V> setThreadPoolAlias(String threadPoolAlias) {
            this.threadPoolAlias = threadPoolAlias;
            return this;
        }

        public EhcacheSolutionOptionTwo.Builder<K, V> setSettings(Settings settings) {
            this.settings = settings;
            return this;
        }

        public EhcacheSolutionOptionTwo.Builder<K, V> setDiskCacheAlias(String diskCacheAlias) {
            this.diskCacheAlias = diskCacheAlias;
            return this;
        }

        public EhcacheSolutionOptionTwo.Builder<K, V> setSettingPrefix(String settingPrefix) {
            this.settingPrefix = settingPrefix;
            return this;
        }

        public EhcacheSolutionOptionTwo.Builder<K, V> setIsEventListenerModeSync(boolean isEventListenerModeSync) {
            this.isEventListenerModeSync = isEventListenerModeSync;
            return this;
        }

        public EhcacheSolutionOptionTwo.Builder<K, V> setKeySerializer(Serializer<K, byte[]> keySerializer) {
            this.keySerializer = keySerializer;
            return this;
        }

        public EhcacheSolutionOptionTwo.Builder<K, V> setValueSerializer(Serializer<V, byte[]> valueSerializer) {
            this.valueSerializer = valueSerializer;
            return this;
        }

        public EhcacheSolutionOptionTwo<K, V> build() {
            System.out.println("Within build()");
            return new EhcacheSolutionOptionTwo<>(this);
        }
    }
